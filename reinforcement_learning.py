...
def greedy_action(q, state):
    ...
    for action in range(q.shape[1]):
        if q[state,action] > q[state,g_action]:
            g_action = np.copy(action)
    return g_action
...
class Sarsa(RLAlgorithm):
    ...
    def get_greedy_action(self, state):
        ...
        :rtype: int.
        """
        return epsilon_greedy_action(self.q, state)
...
class QLearning(RLAlgorithm):
    ...
    def learn(self, state, action, reward, next_state, next_action):
        next_action_ = self.get_greedy_action(state)
        self.q[state,action] = self.q[state,action] + self.alpha*(reward+self.gamma*(self.q[next_state,next_action_])-self.q[state,action])
        pass
